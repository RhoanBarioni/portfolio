...
const DAYS = 119; // 17 weeks of activity
...
export const getGithubStats = createServerFn({ method: "GET" })
  .inputValidator((data: { user: string; pinned?: string[] }) => data)
  .handler(async ({ data }): Promise<GithubStats> => {
    ...
    try {
      ...
      const pinned = repos
        .filter((r) => r.pinned)
        .sort(
          (a, b) => pinnedNames.indexOf(a.name.toLowerCase()) - pinnedNames.indexOf(b.name.toLowerCase()),
        );
